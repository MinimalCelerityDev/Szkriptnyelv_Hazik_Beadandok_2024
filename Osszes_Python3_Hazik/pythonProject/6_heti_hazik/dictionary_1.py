...
def tisztitas(s):
    ...
    szavak_ = re.findall(r"\b\w[\w'-]*\b", s)
...
def main():
    ...
    words = tisztitas(TEXT)

    szavak_szamlaloja = Counter(words)

    rendezettek = dict(sorted(szavak_szamlaloja.items()))

    for szo, szamold in rendezettek.items():
